...
class SubMenuController(
    ...
    private val subMenuListBurger = listOf(
    ...
    private val subMenuListPizza = listOf(
    ...
    private val subMenuListIce = listOf(
    ...
    private val subMenuListBeer = listOf(
    ...
    fun runSub() {
        subMenuList = when (userSelectNumber.mainNumber) {
            ...
            3 -> subMenuListIce
            4 -> subMenuListBeer
            // IllegalStateException : 메소드가 요구한 처리를 수행하기 적합한 상태가 아님
            else -> throw IllegalStateException("[not case] dev: 새로운 메뉴를 추가하셨나요?")
        }
        outputView.printMenuList(
            MENU_LIST[userSelectNumber.mainNumber],
            subMenuList,
            "0. Back          | 뒤로가기\n"
        )
